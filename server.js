// Nickel and Dimed API. Node 22.12+ (Supabase client runtime floor).
//   npm run api
...
import { z } from 'zod';
import { mergeExtractedInvoices, splitInput } from './src/ingest.js';
...
async function extract(text, vendorNames) {
  ...
  for (const chunk of splitInput(text, CHUNK)) {
  ...
  // A multi-line invoice can straddle a model chunk. Rejoin fragments using the
  // same vendor-and-date contract the extraction prompt applies inside a chunk.
  return mergeExtractedInvoices(out);
}

const invoiceSourceHash = (invoice) => createHash('sha256')
  .update(JSON.stringify([
    invoice.invoice_date,
    invoice.source_line.replace(/\r\n/g, '\n').trim(),
  ]))
  .digest('hex');
  ...
  new Promise((resolve, reject) => {
    ...
    req.on('data', (c) => {
      ...
      if (s.length > 5_000_000) {
        req.destroy();
        reject(new HttpError(413, 'input too large; split the file'));
      }
    });
...
const routes = {
  'POST /api/ingest': async (req) => {
    ...
    if (typeof text !== 'string' || !text.trim()) throw new HttpError(400, 'nothing to ingest');
    ...
    const { data, error } = await db.rpc('ingest_invoices', {
      payload: invoices.map(({ source_line, ...i }) => ({
        ...i,
        raw_input: source_line,
        source_hash: invoiceSourceHash({ ...i, source_line }),
      })),
  ...
  'GET /api/dashboard': async () => {
    const [alerts, monthly, items, mergeCandidates] = await Promise.all([
      ...
      db.from('item_monthly')
        ...
        .order('month'),
      db.from('vendor_merge_candidates').select('*'),
    ]);
    for (const r of [alerts, monthly, items, mergeCandidates]) if (r.error) throw r.error;
    return {
      alerts: alerts.data,
      monthly: monthly.data,
      items: items.data,
      mergeCandidates: mergeCandidates.data,
    };
  },
  ...
  'GET /api/vendor': async (req) => {
    const id = Number(new URL(req.url, 'http://localhost').searchParams.get('id'));
    if (!Number.isInteger(id) || id <= 0) throw new HttpError(400, 'bad vendor id');
    ...
    if (!vendor.data) throw new HttpError(404, 'no such vendor');
  ...
  'PATCH /api/invoice': async (req) => {
    ...
    if (body.amount != null) {
      ...
      if (cErr) throw cErr;
      if (count > 0) {
        throw new HttpError(409, 'this invoice is itemized -- correct its line items, not the total');
      }
      const n = Number(body.amount);
      if (!Number.isFinite(n) || n < 0) throw new HttpError(400, 'amount must be a number, and not negative');
      patch.amount = n;
    }
    if (body.invoice_date != null) {
      if (!/^\d{4}-\d{2}-\d{2}$/.test(body.invoice_date)) throw new HttpError(400, 'date must be YYYY-MM-DD');
      const parsed = new Date(`${body.invoice_date}T00:00:00Z`);
      if (Number.isNaN(parsed.valueOf()) || parsed.toISOString().slice(0, 10) !== body.invoice_date) {
        throw new HttpError(400, 'not a real date');
      }
      patch.invoice_date = body.invoice_date;
    }
    if (body.category != null) {
      if (!CATEGORIES.includes(body.category)) throw new HttpError(400, 'unknown category');
  ...
  'PATCH /api/line': async (req) => {
    ...
    const patch = {};
    if (body.item != null) {
      if (typeof body.item !== 'string' || !body.item.trim()) throw new HttpError(400, 'item cannot be empty');
      patch.item = body.item.trim();
    }
    if (body.qty != null) {
      const n = Number(body.qty);
      if (!Number.isFinite(n) || n <= 0) throw new HttpError(400, 'qty must be a positive number');
      patch.qty = n;
    }
    if (body.unit_price != null) {
      const n = Number(body.unit_price);
      if (!Number.isFinite(n) || n < 0) throw new HttpError(400, 'unit price must be a number, and not negative');
      patch.unit_price = n;
    }
    if (!Object.keys(patch).length) throw new HttpError(400, 'nothing to change');
...
http
  .createServer(async (req, res) => {
    ...
    try {
      ...
      send(status, { error: status < 500 ? e.message : 'request failed' });
    }
  })
  // This process holds a Supabase secret key and has destructive routes. It is a
  // local tool until authentication exists, so do not expose it on every interface.
  .listen(3001, '127.0.0.1', () => console.log('nickel-and-dimed api on http://127.0.0.1:3001'));
